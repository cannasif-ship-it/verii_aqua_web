    ...
    return (
      <div className="w-full space-y-6 animate-pulse p-4">
        <div className="flex items-center gap-4">
          <Skeleton className="h-10 w-10 md:h-12 md:w-12 rounded-xl bg-slate-200 dark:bg-white/5" />
          <div className="space-y-2">
             <Skeleton className="h-6 w-32 md:w-48 bg-slate-200 dark:bg-white/5" />
             <Skeleton className="h-4 w-24 md:w-32 bg-slate-200 dark:bg-white/5" />
          </div>
        </div>
        <Skeleton className="h-[400px] md:h-[500px] w-full rounded-3xl bg-slate-200 dark:bg-white/5" />
      </div>
    );
  }
  ...
  if (!stock) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center space-y-6 px-4">
        <div className="relative bg-slate-100 dark:bg-white/5 p-6 rounded-full border border-slate-200 dark:border-white/10 shadow-xl">
          <PackageOpen className="h-10 w-10 text-pink-600" />
        </div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">{t('stock.detail.notFound')}</h2>
        <Button onClick={() => navigate('/stocks')} className="bg-slate-900 text-white dark:bg-white dark:text-zinc-900 rounded-full px-8 w-full md:w-auto hover:bg-slate-800 dark:hover:bg-slate-200">
    ...
    <div className="w-full space-y-6 md:space-y-8 pb-10 px-0 md:px-4">
      <div className="flex items-center gap-4 md:gap-5 px-4 md:px-0">
        <Button
            ...
            className="group h-10 w-10 md:h-12 md:w-12 shrink-0 rounded-xl md:rounded-2xl bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-white/10 transition-colors"
        >
            <ArrowLeft className="h-4 w-4 md:h-5 md:w-5 text-slate-500 dark:text-zinc-500 group-hover:text-pink-600 dark:group-hover:text-pink-500 transition-colors" />
        </Button>
        <div className="space-y-1 min-w-0">
            <h1 className="text-xl md:text-3xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2 md:gap-3 truncate">
                <span className="truncate">{stock.stockName}</span>
                <ShieldCheck className="w-4 h-4 text-emerald-500 shrink-0" />
            </h1>
            <div className="flex items-center gap-2 text-xs md:text-sm text-slate-500 dark:text-zinc-400 font-medium truncate">
                <Box className="w-3 h-3 md:w-4 md:h-4 text-pink-600 dark:text-pink-500 shrink-0" />
                <span className="truncate">{stock.erpStockCode || 'Kod Yok'}</span>
            </div>
        </div>
      </div>

      <div className="relative bg-white dark:bg-[#1a1025]/60 backdrop-blur-xl border-y md:border border-slate-200 dark:border-white/5 rounded-none md:rounded-2xl shadow-sm dark:shadow-xl overflow-hidden transition-colors">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <div className="border-b border-slate-100 dark:border-white/5 px-4 md:px-6 pt-4 overflow-x-auto no-scrollbar">
              ...
              <div className="p-4 md:p-8">
                  <TabsContent value="basic" className="mt-0 animate-in fade-in slide-in-from-bottom-2 duration-500">
                      <div className="flex flex-col lg:flex-row gap-8">
                          <div className="w-full lg:w-1/3 space-y-6 order-2 lg:order-1">
                              <div className="bg-slate-50 dark:bg-white/5 rounded-xl p-5 border border-slate-100 dark:border-white/5 transition-colors">
                                  <h3 className="text-base font-bold mb-4 text-slate-900 dark:text-white flex items-center gap-2">
                                      <Info className="w-4 h-4 text-pink-600 dark:text-pink-500" />
                  ...
                  <TabsContent value="images" className="mt-0 animate-in fade-in slide-in-from-bottom-2 duration-500">
                      <div className="space-y-6">
                          <div className="bg-slate-50 dark:bg-white/5 border-2 border-dashed border-slate-200 dark:border-white/10 rounded-xl p-6 transition-colors">
                  ...
                  <TabsContent value="relations" className="mt-0 animate-in fade-in slide-in-from-bottom-2 duration-500">
                      <div className="flex flex-col lg:flex-row gap-8">
                           <div className="w-full lg:w-1/3 order-2 lg:order-1">
                              <div className="bg-slate-50 dark:bg-white/5 rounded-xl p-5 border border-slate-100 dark:border-white/5 transition-colors">
                                  <h3 className="font-bold text-base mb-4 text-slate-900 dark:text-white">{t('stock.detail.addRelation')}</h3>
...
function TabItem({ value, icon: Icon, label, active }: any) {
  return (
    <TabsTrigger value={value} className={cn(
      ...
      "text-xs md:text-sm text-slate-500 dark:text-zinc-500",
      active && "text-pink-600 border-pink-600 dark:text-pink-500 dark:border-pink-500 font-bold"
    )}>
