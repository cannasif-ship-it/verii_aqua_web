...
import type { NetOperationTypeDto } from '../types/quick-daily-entry-types';
...
export function NetOperationQuickForm({
  ...
  const form = useForm<NetOperationQuickFormSchema>({
    ...
    defaultValues: { netOperationTypeId: 0, description: '' },
  });

  const handleSubmit: SubmitHandler<NetOperationQuickFormSchema> = async (data) => {
    await onSubmit(data);
    form.reset({ netOperationTypeId: 0, description: '' });
  };
