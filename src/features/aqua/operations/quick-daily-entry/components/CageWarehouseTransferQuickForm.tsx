...
import { cageWarehouseTransferQuickFormSchema, type CageWarehouseTransferQuickFormSchema } from '../schema/quick-daily-entry-schema';
import { ChevronRight, Info, Save } from 'lucide-react';

interface CageWarehouseTransferQuickFormProps {
  projectId: number | null;
  projectCageId: number | null;
  warehouseOptions: Array<{ value: string; label: string }>;
  sourceBatch: { fishBatchId: number; liveCount: number; averageGram: number; batchCode?: string } | null;
  onSubmit: (data: CageWarehouseTransferQuickFormSchema) => Promise<void>;
  isSubmitting: boolean;
  canSubmit: boolean;
}

export function CageWarehouseTransferQuickForm({
  projectId,
  projectCageId,
  warehouseOptions,
  sourceBatch,
  ...
  isSubmitting,
  canSubmit,
}: CageWarehouseTransferQuickFormProps): ReactElement {
  const { t } = useTranslation('common');
  const form = useForm<CageWarehouseTransferQuickFormSchema>({
    resolver: zodResolver(cageWarehouseTransferQuickFormSchema) as Resolver<CageWarehouseTransferQuickFormSchema>,
    mode: 'onChange',
    defaultValues: {
      toWarehouseId: 0,
      fishCount: 0,
  ...
  const handleSubmit: SubmitHandler<CageWarehouseTransferQuickFormSchema> = async (data) => {
    await onSubmit(data);
    form.reset({
      toWarehouseId: 0,
    ...
    <Card className="bg-white dark:bg-blue-950/60 backdrop-blur-xl border border-slate-200 dark:border-cyan-800/30 shadow-sm rounded-2xl overflow-hidden transition-all duration-300">
      <CardHeader className="border-b border-slate-200 dark:border-cyan-800/30 px-6 py-5 bg-slate-50/50 dark:bg-blue-950/30">
        <CardTitle className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">
          {t('aqua.quickDailyEntry.cageWarehouseTransfer.title')}
        </CardTitle>
      </CardHeader>
        ...
        {sourceBatch && (
          <div className="mb-6 p-4 rounded-xl bg-cyan-50 dark:bg-cyan-900/20 border border-cyan-100 dark:border-cyan-800/50 flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-cyan-800 dark:text-cyan-300">
              {t('aqua.quickDailyEntry.cageWarehouseTransfer.sourceInfo', {
                ...
                render={({ field }) => (
                  <FormItem className="space-y-2">
                    <FormLabel required className={labelStyle}>
                      ...
                      {t('aqua.quickDailyEntry.cageWarehouseTransfer.targetWarehouse')}
                    </FormLabel>
                    <FormControl>
                      <Combobox
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-2">
                    <FormLabel required className={labelStyle}>
                      <ChevronRight size={14} className="text-cyan-500" />
                      {t('aqua.quickDailyEntry.cageWarehouseTransfer.fishCount')}
                    </FormLabel>
