...
interface WarehouseTransferQuickFormProps {
  ...
  onSubmit: (data: WarehouseTransferQuickFormSchema) => Promise<void>;
  onSourceWarehouseChange: (warehouseId: number | null) => void;
  isSubmitting: boolean;
  canSubmit: boolean;
}

export function WarehouseTransferQuickForm({
  projectId,
  warehouseOptions,
  batchOptions,
  batchSnapshots,
  ...
  isSubmitting,
  canSubmit,
}: WarehouseTransferQuickFormProps): ReactElement {
  const { t } = useTranslation('common');
  const form = useForm<WarehouseTransferQuickFormSchema>({
    resolver: zodResolver(warehouseTransferQuickFormSchema) as Resolver<WarehouseTransferQuickFormSchema>,
    mode: 'onChange',
    defaultValues: {
      fromWarehouseId: 0,
      toWarehouseId: 0,
  ...
  const handleSubmit: SubmitHandler<WarehouseTransferQuickFormSchema> = async (data) => {
    await onSubmit(data);
    form.reset({
    ...
    <Card className="bg-white dark:bg-blue-950/60 backdrop-blur-xl border border-slate-200 dark:border-cyan-800/30 shadow-sm rounded-2xl overflow-hidden transition-all duration-300">
      <CardHeader className="border-b border-slate-200 dark:border-cyan-800/30 px-6 py-5 bg-slate-50/50 dark:bg-blue-950/30">
        <CardTitle className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">
          {t('aqua.quickDailyEntry.warehouseTransfer.title')}
        </CardTitle>
      </CardHeader>
        ...
        {selectedBatch && (
          <div className="mb-6 p-4 rounded-xl bg-cyan-50 dark:bg-cyan-900/20 border border-cyan-100 dark:border-cyan-800/50 flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-cyan-800 dark:text-cyan-300">
              {t('aqua.quickDailyEntry.warehouseTransfer.sourceInfo', {
                ...
                render={({ field }) => (
                  <FormItem className="space-y-2">
                    <FormLabel required className={labelStyle}>
                      ...
                      {t('aqua.quickDailyEntry.warehouseTransfer.sourceWarehouse')}
                    </FormLabel>
                    <FormControl>
                      <Combobox
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-2">
                    <FormLabel required className={labelStyle}>
                      <ChevronRight size={14} className="text-cyan-500" />
                      {t('aqua.quickDailyEntry.warehouseTransfer.targetWarehouse')}
                    </FormLabel>
                    <FormControl>
                      <Combobox
                        options={warehouseOptions}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-2">
                    <FormLabel required className={labelStyle}>
                      <ChevronRight size={14} className="text-cyan-500" />
                      {t('aqua.quickDailyEntry.warehouseTransfer.sourceBatch')}
                    </FormLabel>
                    <FormControl>
                      <Combobox
