...
function CageCard({ cage, onClick, isSelected = false, clickable = false, t }: any): ReactElement {
      ...
      <div className="flex flex-col h-full z-10 relative">
        ...
        <div className="grid grid-cols-2 gap-2 p-3 flex-1">
          <div className="flex flex-col justify-center gap-1 rounded-xl bg-slate-50 dark:bg-blue-900/10 border border-slate-100 dark:border-cyan-800/20 p-2.5">
            <div className="flex items-center gap-1.5"><Skull className="size-3 text-rose-500 shrink-0" /><p className="text-[9px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 truncate">{t('aquaDashboard.cageCard.totalDead', { ns: 'dashboard' })}</p></div>
            <p className="text-xs font-black text-rose-600 dark:text-rose-400 tabular-nums truncate">{formatNumber(cage.totalDeadCount)}</p>
          </div>
          <div className="flex flex-col justify-center gap-1 rounded-xl bg-slate-50 dark:bg-blue-900/10 border border-slate-100 dark:border-cyan-800/20 p-2.5">
            <div className="flex items-center gap-1.5"><UtensilsCrossed className="size-3 text-amber-500 shrink-0" /><p className="text-[9px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 truncate">{t('aquaDashboard.cageCard.feed', { ns: 'dashboard' })}</p></div>
          ...
          <div className="col-span-2 flex flex-col justify-center gap-1 rounded-xl bg-slate-50 dark:bg-blue-900/10 border border-slate-100 dark:border-cyan-800/20 p-2.5">
            <div className="flex items-center gap-1.5"><Layers className="size-3 text-blue-500 shrink-0" /><p className="text-[9px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 truncate">{t('aquaDashboard.cageCard.biomass', { ns: 'dashboard' })}</p></div>
        ...
        <div className="px-4 pb-4">
          <div className="flex justify-between items-end mb-1"><span className="text-[9px] font-bold uppercase tracking-widest text-slate-400">{t('aquaDashboard.cageCard.survivalRate', { ns: 'dashboard' })}</span><span className={cn("text-[10px] font-black", isCritical ? "text-rose-500" : isWarning ? "text-amber-500" : "text-emerald-500")}>%{survivalRate.toFixed(1)}</span></div>
    ...
    <div className="space-y-8 pb-10 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-1">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-900 dark:text-white flex items-center gap-3">
            ...
            {t('aquaDashboard.title', { ns: 'dashboard' })}
          </h1>
              ...
              <Button variant="outline" className="h-12 w-full md:w-auto bg-white/80 dark:bg-blue-950/60 backdrop-blur-xl border-slate-200 dark:border-cyan-800/50 shadow-sm hover:shadow-lg dark:hover:border-cyan-600 rounded-xl px-5 flex items-center gap-4 transition-all">
                ...
                <div className="flex flex-col items-start text-left">
                  <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest leading-none mb-1">
                    {t('aquaDashboard.controls.viewAndProject', { ns: 'dashboard' })}
                  </span>
                  <span className="text-[13px] font-black text-slate-700 dark:text-slate-200 leading-tight">
                    {t('aquaDashboard.controls.projectCountAndView', {
                      ns: 'dashboard',
                      count: activeDashboardProjectIds.length,
                      view: viewMode === 'card'
                        ? t('aquaDashboard.controls.cardView', { ns: 'dashboard' })
                        : t('aquaDashboard.controls.listView', { ns: 'dashboard' }),
                    })}
                  </span>
                </div>
            ...
            <PopoverContent align="end" className="w-[340px] p-0 bg-white/95 dark:bg-blue-950/95 backdrop-blur-xl border border-slate-200 dark:border-cyan-800/50 shadow-2xl rounded-2xl overflow-hidden z-50">
              ...
              <div className="p-4 border-b border-slate-100 dark:border-cyan-800/30 bg-slate-50/50 dark:bg-blue-900/10">
                <p className="text-[10px] font-bold text-slate-500 dark:text-cyan-500 uppercase tracking-widest mb-3 ml-1 flex items-center gap-2">
                  <LayoutGrid className="w-3 h-3" /> {t('aquaDashboard.controls.viewType', { ns: 'dashboard' })}
                </p>
                <div className="flex bg-slate-200/50 dark:bg-blue-950 p-1 rounded-xl border border-slate-200 dark:border-cyan-800/50 w-full h-10 items-center relative">
                  <button onClick={() => setViewMode('card')} className={cn("flex-1 h-full rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all relative z-10", viewMode === 'card' ? "bg-white dark:bg-cyan-500 text-cyan-600 dark:text-white shadow-sm" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-cyan-300")}>
                    <LayoutGrid size={14} /> {t('aquaDashboard.controls.cardView', { ns: 'dashboard' })}
                  </button>
                  <button onClick={() => setViewMode('list')} className={cn("flex-1 h-full rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all relative z-10", viewMode === 'list' ? "bg-white dark:bg-cyan-500 text-cyan-600 dark:text-white shadow-sm" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-cyan-300")}>
                    <List size={14} /> {t('aquaDashboard.controls.listView', { ns: 'dashboard' })}
                  </button>
                </div>
              </div>
              ...
              <div className="p-3 border-b border-slate-100 dark:border-cyan-800/30 flex items-center justify-between bg-white dark:bg-transparent">
                 <p className="text-[10px] font-bold text-slate-500 dark:text-pink-500 uppercase tracking-widest ml-2 flex items-center gap-2">
                   <Layers className="w-3 h-3" /> {t('aquaDashboard.controls.projectSelection', { ns: 'dashboard' })}
                 </p>
                 <div className="flex items-center gap-1">
                   <Button variant="ghost" size="sm" onClick={() => setActiveDashboardProjectIds(summaries.map(s => s.projectId))} className="h-6 px-2 text-[10px] font-bold text-cyan-600 dark:text-cyan-400 hover:bg-cyan-50 dark:hover:bg-cyan-900/30 rounded-md">{t('aquaDashboard.controls.selectAll', { ns: 'dashboard' })}</Button>
                   <Button variant="ghost" size="sm" onClick={() => setActiveDashboardProjectIds([])} className="h-6 px-2 text-[10px] font-bold text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-md">{t('aquaDashboard.controls.clear', { ns: 'dashboard' })}</Button>
                 </div>
              </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6">
          {[
            { label: t('aquaDashboard.stats.activeCages', { ns: 'dashboard' }), val: globalStats.totalCages, icon: Waves, color: 'text-cyan-600 dark:text-cyan-400', bg: 'bg-cyan-50 dark:bg-cyan-500/10 border-cyan-100 dark:border-cyan-800/30' },
            { label: t('aquaDashboard.stats.liveFish', { ns: 'dashboard' }), val: globalStats.totalFish, icon: Fish, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-100 dark:border-emerald-800/30' },
            { label: t('aquaDashboard.stats.totalBiomass', { ns: 'dashboard' }), val: `${formatNumber(globalStats.totalBio)}g`, icon: Layers, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-500/10 border-blue-100 dark:border-blue-800/30' },
            { label: t('aquaDashboard.stats.totalFeed', { ns: 'dashboard' }), val: `${formatNumber(globalStats.totalFeed)}g`, icon: UtensilsCrossed, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-500/10 border-amber-100 dark:border-amber-800/30' },
          ].map((stat, i) => (
              ...
              <CardHeader className="border-b border-slate-100 dark:border-cyan-800/30 bg-slate-50/80 dark:bg-blue-900/20 px-6 py-5 transition-colors">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <Button type="button" className="bg-linear-to-r from-pink-600 to-orange-600 text-white h-10 px-6 rounded-xl font-bold shadow-lg shadow-pink-500/20 border-0 transition-transform hover:scale-105" onClick={() => openProjectDetail(project.projectId)}>
                    <BarChart3 className="size-4 mr-2" /> {t('aquaDashboard.detailReportButton', { ns: 'dashboard' })}
                  </Button>
                </div>
              </CardHeader>
                ...
                {viewMode === 'card' ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                    {project.cages.map((cage: any) => (
                      <CageCard key={cage.projectCageId} cage={cage} clickable onClick={() => openDailyFromProjectList(project.projectId, cage.projectCageId)} t={t} />
                    ))}
                  </div>
                ) : (
          ...
          <DialogHeader className="border-b border-slate-200 dark:border-cyan-800/30 bg-slate-50 dark:bg-blue-950/50 px-6 py-5 sm:px-8 shrink-0 relative">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div><DialogTitle className="text-xl font-black text-slate-900 dark:text-white">{selectedCageFromDetail?.cageLabel ?? '-'}</DialogTitle><p className="text-cyan-600 dark:text-cyan-400 text-sm font-bold flex items-center gap-2 mt-0.5"><span className="w-1.5 h-1.5 rounded-full bg-cyan-500 animate-pulse" />{t('aquaDashboard.dailyFlow', { ns: 'dashboard' })}</p></div>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className="flex-1 overflow-x-auto p-4 sm:p-8 custom-scrollbar bg-slate-50/50 dark:bg-black/20">
            <div className="min-w-max rounded-2xl border border-slate-200 dark:border-cyan-800/30 bg-white dark:bg-blue-950/40 overflow-hidden shadow-xl">
              <div className={cn(TABLE_GRID_LAYOUT, "px-6 py-4 bg-slate-100/80 dark:bg-blue-900/40 border-b border-slate-200 dark:border-cyan-800/30 text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest")}>
                <span>{t('aquaDashboard.dailyTable.date', { ns: 'dashboard' })}</span><span className="text-center">{t('aquaDashboard.dailyTable.feedGram', { ns: 'dashboard' })}</span><span className="text-center">{t('aquaDashboard.dailyTable.stock', { ns: 'dashboard' })}</span><span className="text-center text-rose-500">{t('aquaDashboard.dailyTable.dead', { ns: 'dashboard' })}</span><span className="text-center">{t('aquaDashboard.dailyTable.delta', { ns: 'dashboard' })}</span><span className="text-center text-blue-500">{t('aquaDashboard.dailyTable.biomassDelta', { ns: 'dashboard' })}</span><span>{t('aquaDashboard.dailyTable.weather', { ns: 'dashboard' })}</span><span className="text-center">{t('aquaDashboard.dailyTable.net', { ns: 'dashboard' })}</span><span className="text-center">{t('aquaDashboard.dailyTable.transfer', { ns: 'dashboard' })}</span><span className="text-center">{t('aquaDashboard.dailyTable.shipment', { ns: 'dashboard' })}</span><span className="text-right">{t('aquaDashboard.dailyTable.status', { ns: 'dashboard' })}</span>
              </div>
                ...
                {selectedCageFromDetail?.dailyRows.map((row: any, index: number) => {
                  ...
                  return (
                    <div key={row.date} className={cn(TABLE_GRID_LAYOUT, "px-6 py-4 hover:bg-slate-50 dark:hover:bg-blue-900/20 transition-all text-[13px]", isHighDead && "bg-rose-500/5")}>
                      ...
                      <div className="flex justify-center">{row.netOperationCount > 0 ? (<button onClick={() => openDetailDialog(selectedCageFromDetail, 'netOperation')(row.date, row.netOperationDetails)} className="text-[11px] text-cyan-600 dark:text-cyan-400 font-black hover:underline transition-all">{t('aquaDashboard.dailyTable.operationCount', { ns: 'dashboard', count: row.netOperationCount })}</button>) : <span className="text-slate-300 dark:text-slate-700">-</span>}</div>
                      ...
                      <div className="flex justify-end"><Badge className={cn("px-2.5 py-1 font-black text-[10px] rounded-lg border-0 shadow-sm", row.fed ? "bg-emerald-500/10 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400" : "bg-rose-500/10 text-rose-600 dark:bg-rose-500/20 dark:text-rose-400")}>{row.fed ? t('aquaDashboard.dailyTable.fed', { ns: 'dashboard' }) : t('aquaDashboard.dailyTable.notFed', { ns: 'dashboard' })}</Badge></div>
                    </div>
                  );
                })}
              </div>
        ...
        <DialogContent className="w-[95vw] max-w-4xl max-h-[90dvh] overflow-hidden bg-white dark:bg-blue-950 border-slate-200 dark:border-cyan-800/30 p-0 rounded-4xl shadow-2xl flex flex-col outline-none">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6 lg:space-y-8 custom-scrollbar">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 dark:border-emerald-500/20 dark:bg-emerald-500/5 p-4 flex flex-col justify-center"><p className="text-[10px] font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400/80 mb-1">{t('aquaDashboard.projectDialog.fish', { ns: 'dashboard' })}</p><p className="text-xl sm:text-2xl font-black text-emerald-700 dark:text-emerald-400 truncate tabular-nums">{formatNumber(detailQuery.data?.cages.reduce((acc, cage) => acc + cage.currentFishCount, 0) || 0)}</p></div>
              <div className="rounded-2xl border border-rose-200 bg-rose-50 dark:border-rose-500/20 dark:bg-rose-500/5 p-4 flex flex-col justify-center"><p className="text-[10px] font-bold uppercase tracking-widest text-rose-600 dark:text-rose-400/80 mb-1">{t('aquaDashboard.projectDialog.dead', { ns: 'dashboard' })}</p><p className="text-xl sm:text-2xl font-black text-rose-700 dark:text-rose-400 truncate tabular-nums">{formatNumber(detailQuery.data?.cages.reduce((acc, cage) => acc + cage.totalDeadCount, 0) || 0)}</p></div>
              <div className="rounded-2xl border border-amber-200 bg-amber-50 dark:border-amber-500/20 dark:bg-amber-500/5 p-4 flex flex-col justify-center"><p className="text-[10px] font-bold uppercase tracking-widest text-amber-600 dark:text-amber-400/80 mb-1">{t('aquaDashboard.projectDialog.feedGram', { ns: 'dashboard' })}</p><p className="text-xl sm:text-2xl font-black text-amber-700 dark:text-amber-400 truncate tabular-nums">{formatNumber(detailQuery.data?.cages.reduce((acc, cage) => acc + cage.totalFeedGram, 0) || 0)}</p></div>
              <div className="rounded-2xl border border-blue-200 bg-blue-50 dark:border-blue-500/20 dark:bg-blue-500/5 p-4 flex flex-col justify-center"><p className="text-[10px] font-bold uppercase tracking-widest text-blue-600 dark:text-blue-400/80 mb-1">{t('aquaDashboard.projectDialog.cages', { ns: 'dashboard' })}</p><p className="text-xl sm:text-2xl font-black text-blue-700 dark:text-blue-400 truncate tabular-nums">{formatNumber(detailQuery.data?.cages.length || 0)}</p></div>
            </div>
            <div className="space-y-4">
              <div className="flex items-center gap-3 px-1"><div className="p-1.5 bg-cyan-100 dark:bg-cyan-500/20 rounded-lg"><Layers className="size-4 text-cyan-600 dark:text-cyan-400" /></div><h3 className="text-lg font-black text-slate-900 dark:text-white tracking-tight">{t('aquaDashboard.projectDialog.cagePools', { ns: 'dashboard' })}</h3><Badge variant="secondary" className="text-[10px] font-bold bg-slate-100 text-slate-500 dark:bg-blue-900/30 dark:text-slate-400 border-0 ml-2 px-3 py-1">{t('aquaDashboard.projectDialog.clickForDailyDetail', { ns: 'dashboard' })}</Badge></div>
              {detailQuery.isLoading ? <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2"><CageSkeleton /><CageSkeleton /></div> : <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{detailQuery.data?.cages.map((cage) => (<CageCard key={cage.projectCageId} cage={cage} clickable isSelected={selectedCageId === cage.projectCageId} onClick={() => openCageDailyDialog(cage.projectCageId)} t={t} />))}</div>}
            </div>
          </div>
        </DialogContent>
      </Dialog>
