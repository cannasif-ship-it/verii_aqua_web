...
export function AquaDashboardPage(): ReactElement {
  ...
  // {t('aquaDashboard.facilitySummary', { ns: 'dashboard' })} Hesaplamaları (Tüm Projeler)
  ...
  const openDetailDialog = (cage: any, type: DetailType): ((date: string, items: string[]) => void) => {
    const titleMap: Record<DetailType, string> = { feeding: t('aquaDashboard.detailTypes.feeding', { ns: 'dashboard' }), netOperation: t('aquaDashboard.detailTypes.netOperation', { ns: 'dashboard' }), transfer: t('aquaDashboard.detailTypes.transfer', { ns: 'dashboard' }), stockConvert: t('aquaDashboard.detailTypes.stockConvert', { ns: 'dashboard' }), shipment: t('aquaDashboard.detailTypes.shipment', { ns: 'dashboard' }) };
    return (date: string, items: string[]) => {
      if (items.length === 0) return;
      setDetailDialog({ open: true, title: `${titleMap[type]} - ${cage.cageLabel}`, description: t('aquaDashboard.detailDialog.date', { ns: 'dashboard', date }), items });
    };
  };
    ...
    <div className="space-y-8 pb-10 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-1">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm font-medium ml-1">{t('aquaDashboard.description', { ns: 'dashboard' })}</p>
        </div>
        ...
        <div className="w-full md:w-[350px]">
          <label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 dark:text-cyan-400 mb-2 block ml-1">
            {t('aquaDashboard.selectedProject', { ns: 'dashboard' })}
          </label>
          <Combobox
            ...
            placeholder={t('aquaDashboard.selectProjectPlaceholder', { ns: 'dashboard' })}
      ...
      <div className="space-y-4 pt-4">
        <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200 px-1 flex items-center gap-2">
          ...
          {t('aquaDashboard.projectDetail', { ns: 'dashboard' })}
        </h2>
