  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={cn(
        ...
        darkMode ? "bg-[#0b0713] text-white" : "bg-white text-slate-900"
      )}>
        <DialogPrimitive.Close className={cn(
          "absolute right-4 top-4 md:right-6 md:top-6 z-50 rounded-xl p-2.5 transition-all duration-200",
          "active:scale-90",
          darkMode 
            ? "bg-white/5 text-slate-400 hover:bg-rose-600 hover:text-white" 
        ...
        <div className="flex flex-col md:flex-row w-full h-full overflow-y-auto md:overflow-hidden">
          ...
          {/* SOL PANEL (Avatar): shrink-0 ile ezilmesi engellendi */}
          <div className={cn(
            ...
            darkMode ? "bg-blue-950/40 border-cyan-800/20" : "bg-slate-50/80 border-slate-200"
          )}>
            ...
            <div className="relative group mb-4 md:mb-6 mt-4 md:mt-0">
              ...
              <div className="absolute -bottom-1 -right-1 w-6 h-6 md:w-8 md:h-8 bg-emerald-500 rounded-xl border-4 border-slate-50 dark:border-[#0b0713] flex items-center justify-center shadow-lg">
            ...
            <div className="w-full mt-6 md:mt-8 flex justify-center z-10">
              <div className={cn("inline-flex items-center gap-3 p-2.5 px-4 rounded-xl transition-all border", darkMode ? "bg-blue-900/20 border-cyan-800/20" : "bg-white border-slate-200 shadow-sm")}>
          ...
          <div className="flex-1 p-6 md:p-10 lg:p-12 flex flex-col min-h-0 relative">
            ...
            <div className={cn(
              ...
              <button
                className={cn(
                  "group w-full p-4 md:p-5 flex items-center justify-between border rounded-3xl transition-all duration-300",
                  darkMode ? "border-cyan-800/20 bg-blue-900/10 hover:border-cyan-500/40 hover:bg-blue-900/20" : "border-slate-200 bg-slate-50/50 hover:bg-white hover:shadow-lg hover:border-cyan-200"
                )}
                onClick={onOpenProfileDetails}
              >
                <div className="flex items-center gap-4">
                  <div className={cn("p-3 rounded-2xl shadow-sm", darkMode ? "bg-blue-500/20 text-blue-400" : "bg-blue-100 text-blue-600")}>
                    <UserIcon size={20} />
                  </div>
                  <div className="text-left">
                    <p className="font-bold text-sm md:text-base text-slate-800 dark:text-slate-200">{t('profile.title')}</p>
                    <p className="text-[10px] md:text-xs text-slate-500 dark:text-slate-400 font-medium">{t('customerManagement.form.editDescription')}</p>
                  </div>
                </div>
              ...
              <div className={cn(
                "group w-full p-4 md:p-5 flex items-center justify-between border rounded-3xl transition-all",
                darkMode ? "border-cyan-800/20 bg-blue-900/10" : "border-slate-200 bg-slate-50/50"
              )}>
                <div className="flex items-center gap-4 flex-1">
                  <div className={cn("p-3 rounded-2xl shadow-sm", darkMode ? "bg-cyan-500/20 text-cyan-400" : "bg-cyan-100 text-cyan-600")}>
                    <LanguageSquareIcon size={20} />
                  </div>
                  <p className="font-bold text-sm md:text-base text-slate-800 dark:text-slate-200">{t('language_choice')}</p>
                </div>
                <Combobox
                  ...
                  className={cn(
                    "w-24 md:w-28 h-10 shadow-sm focus:ring-0 font-bold text-xs transition-all rounded-xl",
                    darkMode ? "bg-blue-950/60 border-cyan-800/30 hover:bg-blue-900/50" : "bg-white border-slate-200 hover:bg-slate-50 text-slate-700"
                  )}
                />
              </div>
              ...
              <div className={cn(
                ...
                <div className="flex items-center gap-4">
                  <div className={cn("p-3 rounded-2xl shadow-sm", darkMode ? "bg-amber-500/20 text-amber-400" : "bg-amber-100 text-amber-600")}>
                    {darkMode ? <Moon02Icon size={20} /> : <Sun01Icon size={20} />}
                  </div>
                  <p className="font-bold text-sm md:text-base text-slate-800 dark:text-slate-200">{t('appearance')}</p>
                </div>
                <Switch 
                  checked={darkMode} 
            ...
            <div className="mt-6 md:mt-8 pt-6 border-t border-slate-200 dark:border-cyan-800/20 shrink-0">
              <Button
                className="w-full h-12 md:h-14 rounded-2xl text-white font-black text-sm md:text-base bg-linear-to-r from-rose-500 to-rose-600 hover:from-rose-600 hover:to-rose-700 hover:scale-[1.01] active:scale-[0.98] transition-all shadow-lg shadow-rose-500/20 border-0"
                onClick={handleLogout}
              >
                <Logout02Icon size={20} className="mr-3" />
