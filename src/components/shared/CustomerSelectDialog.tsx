...
import { dropdownApi, type DropdownCustomerDto } from '@/components/shared/dropdown/dropdown-api';
...
export function CustomerSelectDialog({
  ...
  const {
    ...
    isFetchingNextPage,
    fetchNextPage,
  } = useDropdownInfiniteSearch<DropdownCustomerDto>({
    entityKey: 'customers',
  ...
  const displayCustomers = useMemo(() => {
    const isErp = (c: DropdownCustomerDto): boolean =>
      c.isIntegrated === true ||
  ...
  const handleCustomerSelect = (customer: DropdownCustomerDto & { type: 'erp' | 'crm' }): void => {
    const code =
  ...
  const renderCustomerList = (
    list: Array<DropdownCustomerDto & { type: 'erp' | 'crm' }>,
    emptyKey: string
  ): ReactElement => {
